...
class Game extends EventEmitter {
    ...
    checkCardInPositionOfCards(card, position, cardsCronology) {
        ...
        this.log(`actual: ${card.year} : ${position} ${JSON.stringify(playerCardsSortedByYear)}`);
    ...
    selectCardInPlayer(position, otherPlayers) {
        ...
        otherPlayers.forEach(bid => {
            ...
            if (this.checkCardInPositionOfCards(card, bid.position, this.players[this.activePlayer].cronology) && (!cardAssigned)) {
    ...
    bidActivePlayer(position) {
        ...
        if ((player.comodins <= 0) && (this.activeBidPlayer !== this.activePlayer)) {
